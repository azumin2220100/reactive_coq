...
(* 有限列と無限列の結合 *)
Fixpoint list_plus_colist
  (U : Type) (l1 : list U) (l2 : colist U) : colist U :=
  match l1 with
    | cons hd tl =>
      cocons U hd (list_plus_colist U tl l2)
    | nil =>
      l2
  end.

Arguments list_plus_colist {U} _ _.
Notation "x @ y" := (list_plus_colist x y)
...
CoFixpoint R_omega'
  ...
  | cocons hd tl =>
      ...
      cocons O (r [hd]) (R_omega' r new_pref tl)
  end.
...
CoFixpoint stepwise_union
  (l1 : colist I)(l2 : colist O) : colist I_cup_O :=
  match l1 with
    | cocons hd1 tl1 =>
      match l2 with
        | cocons hd2 tl2 =>
          cocons _ (hd1 ∪ hd2) (stepwise_union tl1 tl2)
      end
  end.
...
(* 階層定理の証明に用いる定義 *)
CoFixpoint empty_list : colist I :=
  cocons _ ε empty_list.
...
Fixpoint list_suff (U : Type) (l : colist U) (m : nat) : colist U :=
  match m with
    | S n => match l with
               | cocons hd tl => list_suff U tl n
             end
    | 0 => l
  end.
...
Lemma R_omega_calc :
  forall r : R, forall a_bar : list I, forall a_hat : colist I,
    R_omega r (a_bar @ a_hat) = 
      ((R_omega_fin r a_bar) @ (list_suff O (R_omega r(a_bar @ a_hat)) (length a_bar))).
Proof.
Admitted.
...
Definition PresSat : Prop :=
  exists r : R,
    forall a_bar : list I,
      exists a_hat : colist I,
        exists b_hat : colist O,
...
Theorem Real_to_StabStrSat :
  Real -> StabStrSat.
Proof.
  unfold Real, StabStrSat.
  intro H.
  destruct H as [r H].
  exists 0.
  intro m.
  intro a_bar.
  set (r' := fun a_bar' : list I => r (a_bar ⊕ a_bar')).
  exists (R_omega_fin r a_bar).
  destruct H0 as [H2 H3].
  split.
    + pose proof (R_omega_fin'_eq_length a_bar nil r) as H1.
      rewrite -> H3 in H1.
      unfold R_omega_fin. trivial.
    + exists r'.
      intro a_hat.
      specialize(H (a_bar @ a_hat)).
      pose proof (R_omega_calc r a_bar a_hat) as H4.
      rewrite H3 in H4.
      rewrite H4 in H.
      assert(H5 : R_omega r' a_hat = list_suff O (R_omega r (a_bar @ a_hat)) m).
        - unfold r'.
          simpl.
Abort.
